...
/*
   1. noCache 不使用缓存，全部走网络
    ...
    8. FORCE_CACHE 只走缓存*/
    /**
     * 实现了3个拦截器（Interceptor），分别是logging 、baseInterceptor 和 rewriteCacheControlInterceptor。
     * 三个拦截器分别有不同的作用，logging 实现是把请求参数和 response 作为控制台日志方式打印出来；
     * baseInterceptor 是根据请求环境进行不同策略的操作（比如断网环境下的策略）；rewriteCacheControlInterceptor 是网络层面的操作。
     */
    ...
    /*
    主要负责创建具体Retrofit，和调度分发请求。缓存
    设置格式工厂。添加cookie同步，构建OkHttpClient，
    添加BaseUrl
     */
    //构造方法私有
    ...
    /**
     * 云端响应头拦截器，用来配置缓存策略
     * Dangerous interceptor that rewrites the server'ic_girl_normal cache-control header.
     */
